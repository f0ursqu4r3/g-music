...
describe('MCP wire contracts', () => {
  ...
  beforeEach(async () => {
    ...
    server = createServer((socket) => {
      ...
      socket.on('data', (chunk) => {
        ...
        socket.end(`${JSON.stringify({ id: request.id, result: { tracks: [] } })}\n`)
      })
    })
  ...
  it('nests batch metadata exactly as the Rust agent protocol requires', async () => {
    const result = await client.callTool({
      ...
      arguments: {
        confirmed: true,
        updates: [{ id: 'fixture', title: 'Title', artist: 'Artist', genres: [] }],
      },
    })
    expect(result.isError).not.toBe(true)
    expect(requests).toHaveLength(1)
...
describe('Cross-language MCP ↔ Rust dispatch registration', () => {
  it('every invokeAgent method in gmusic-mcp.ts is handled in agent.rs dispatch', async () => {
    ...
    const agentSource = await readFile(resolve('src-tauri/src/agent.rs'), 'utf8')

    // Extract all string literals passed as first argument to invokeAgent(...)
    const mcpMethods = Array.from(mcpSource.matchAll(/invokeAgent\(\s*"([^"]+)"/g)).map((m) => m[1])

    // Extract all string arms in the Rust dispatch match
    const rustArms = Array.from(agentSource.matchAll(/"([a-z]+\.[a-z]+)"\s*=>/g)).map((m) => m[1])

    // Every method the MCP layer calls must have a Rust handler
    const unregistered = mcpMethods.filter((method) => !rustArms.includes(method))
