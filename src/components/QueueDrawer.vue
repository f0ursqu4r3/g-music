...
<template>
  ...
  >
    ...
    <ol class="grid list-none gap-0.75 pt-2.5">
      <ContextMenu v-for="(item, index) in queue" :key="item.id">
        <ContextMenuTrigger as-child>
          <li
            class="group grid min-h-10.75 grid-cols-[25px_minmax(0,1fr)_36px_79px] items-center gap-1.75 rounded-[10px] px-1.75 py-1.5 hover:bg-(--surface-muted) data-[current=true]:bg-(--accent-soft) data-[current=true]:outline data-[current=true]:outline-[color-mix(in_oklch,var(--accent)_38%,transparent)]"
