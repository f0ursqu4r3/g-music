...
import type { EditableTrackMetadata, MediaItem, TrackMetadataUpdate } from '@/api'
...
const entityLabel = computed(() => props.target.kind[0]!.toUpperCase() + props.target.kind.slice(1))
function stringValue(track: MediaItem, key: Field): string {
  return key === 'genres' ? (track.genres ?? []).join(', ') : (track[key] ?? '')
}
watch(
  () => props.target,
  (target) => {
    ...
    for (const { key } of fields) {
      ...
      if (target.tracks.some((track) => stringValue(track, key) !== first)) mixed.add(key)
...
<template>
  <LibraryDialog :title="`Edit ${entityLabel}`" :busy="busy" @close="emit('cancel')">
    <form novalidate @submit.prevent="save">
      ...
      <p v-if="target.tracks.length > 1" class="mt-3 text-xs text-(--muted-text)">
        Only changed fields apply to {{ target.tracks.length }} tracks. Mixed values stay unchanged
        unless edited or cleared.
      </p>
      <div class="mt-4 grid gap-3">
        <label v-for="field in visibleFields" :key="field.key" class="grid gap-1 text-sm">
      ...
      <div v-if="confirmingReset" class="mt-4 rounded-md border border-(--line-strong) p-3 text-sm">
        <p>
        ...
        <div class="mt-3 flex flex-wrap gap-3">
          ...
          >
            Keep editing</button
          ><button data-confirm-metadata-reset type="button" :disabled="busy" @click="reset">
