...
describe("metadata drafts", () => {
  ...
  it("shows mixed fields and saves only dirty values while preserving each hidden value", async () => {
    ...
    await flushPromises();
    expect(
      body().get('[data-metadata-field="label"]').attributes("placeholder"),
    ).toBe("Mixed values");
  ...
  it("does not submit unchanged metadata and supports explicit optional field clears", async () => {
    ...
    await body().get("form").trigger("submit");
    await flushPromises();
    expect(writes[0]?.map((update) => update.metadata.label)).toEqual([
      null,
      null,
    ]);
    expect(writes[0]?.map((update) => update.metadata)).toEqual([
      { label: null },
      { label: null },
    ]);
    wrapper.unmount();
  ...
  it("keeps a failed draft, rejects required clears, and resets only after explicit discard", async () => {
    ...
    await body().get("form").trigger("submit");
    expect(body().get('[role="alert"]').text()).toContain(
    ...
    expect(
      (body().get('[data-metadata-field="title"]').element as HTMLInputElement)
        .value,
    ).toBe("Draft");
    await body().get("[data-metadata-reset]").trigger("click");
    expect(resetIds).toEqual([]);
    expect(body().get("[data-confirm-metadata-reset]").text()).toContain(
...
describe("playlist drafts", () => {
  ...
  it("keeps failed playlist edits and focus inside the dialog", async () => {
    ...
    expect(
      (body().get('[data-playlist-field="name"]').element as HTMLInputElement)
        .value,
    ).toBe("Draft");
    ...
    expect(
      body().get('[role="dialog"]').element.contains(document.activeElement),
    ).toBe(true);
