...
describe('LibrarySidebar', () => {
  ...
  it('clips long playlist names within the sidebar', () => {
    ...
    expect(playlist.classes()).toEqual(expect.arrayContaining(['min-w-0', 'overflow-hidden']))
    expect(title.classes()).toEqual(expect.arrayContaining(['min-w-0', 'flex-1', 'truncate']))
  })
  ...
  it('uses square full-bleed highlights for every sidebar destination', () => {
    ...
    const defaultPlaylist = wrapper.get('[data-default-playlist-row="favorites"]')
  ...
  it('uses full-bleed square highlights in context menus', async () => {
    ...
    const menu = document.body.querySelector<HTMLElement>('[data-playlist-context-menu]')
    const item = menu?.querySelector<HTMLElement>('[data-slot="context-menu-item"]')
  ...
  it('commits a dragged user playlist title order', async () => {
    ...
    await reorderGroup.vm.$emit('update:values', [playlists[3]!, playlists[2]!])
  ...
  it('uses drag reorder without arrow controls', () => {
    ...
    expect(wrapper.find('[aria-label="Move Road Trip down"]').exists()).toBe(false)
  })
})
