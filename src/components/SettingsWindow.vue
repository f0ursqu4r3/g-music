...
import { CircleAlert, ExternalLink, Link2, LogOut, Youtube } from 'lucide-vue-next'
...
async function backUpLibrary(): Promise<void> {
  ...
  try {
    ...
    if (isAlive)
      backupError.value = readError(error, 'Backup failed. Check free disk space and retry.')
  } finally {
...
async function copyText(value: string, success: string): Promise<void> {
  ...
  try {
    ...
    if (isAlive)
      copyError.value = readError(error, 'Copy failed. Select the text and copy it manually.')
  }
}
...
async function updateStatus(operation: () => Promise<YouTubeAuthStatus>): Promise<void> {
  isBusy.value = true
  errorMessage.value = ''
  try {
    ...
    <div class="mx-auto grid max-w-2xl gap-6">
      <header class="grid gap-1">
        ...
        <p class="window-copy m-0 max-w-xl text-sm leading-6">
          Manage appearance, playback readiness, library backups, and your local YouTube session.
        </p>
      </header>
      ...
      <section class="window-panel p-5" aria-labelledby="diagnostics-heading">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h2 id="diagnostics-heading" class="text-base font-semibold">Playback readiness</h2>
          <Button
            type="button"
            variant="outline"
        ...
        <p v-if="isInspecting" role="status" class="mt-2 text-sm text-(--muted-text)">
        ...
        <template v-if="diagnostics">
          ...
          <ul class="mt-3 divide-y divide-(--line)">
            ...
            >
              <p class="font-medium">
                ...
                }}<template v-if="dependency.version"> · {{ dependency.version }}</template>
              </p>
              <p class="mt-1 break-words text-xs text-(--muted-text)">
                {{ dependency.message }}
              </p>
              <p v-if="!dependency.available" class="mt-1 text-xs text-(--muted-text)">
                Install {{ dependency.name }} with your system package manager, then refresh
                diagnostics.
              </p>
            </li>
          </ul>
          ...
          <p class="mt-2 text-sm text-(--muted-text)">
            Select the system default output in your operating system sound settings. If sound stops
            after a device change, pause playback, check the output device, then resume playback.
          </p>
          <Button
            type="button"
            variant="outline"
            ...
            class="mt-3"
            @click="copyText(JSON.stringify(diagnostics, null, 2), 'Support info copied.')"
      ...
      <section class="window-panel p-5" aria-labelledby="backup-heading">
        <h2 id="backup-heading" class="text-base font-semibold">Library backup</h2>
        <p class="mt-1 text-sm text-(--muted-text)">
          Save a consistent library backup. Account sessions and cached media are excluded.
        </p>
        <Button
          type="button"
          variant="outline"
        ...
        <p v-if="backupError" role="alert" class="window-alert-danger mt-3 break-words p-3 text-sm">
          {{ backupError }}
        </p>
      </section>

      <p v-if="copyMessage" role="status" class="text-sm">{{ copyMessage }}</p>
      <p v-if="copyError" role="alert" class="window-alert-danger break-words p-3 text-sm">
        {{ copyError }}
      </p>
      <div v-if="playbackError" class="window-alert-danger p-3 text-sm">
        <p role="alert" class="break-words">{{ playbackError }}</p>
        <Button type="button" variant="outline" class="mt-2" @click="emit('retry')"
      ...
      >
        <div class="flex items-start gap-4 p-5">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 id="youtube-account-heading" class="m-0 text-base font-semibold">
                  YouTube account
                </h2>
                <p
                  :data-auth-state="status.connected ? 'connected' : 'disconnected'"
        ...
        <div v-if="!status.connected" class="grid gap-4 border-t border-(--line) p-5">
          <ol class="m-0 grid list-none gap-3 p-0 text-sm text-(--muted-text)">
            ...
            <li class="flex items-start gap-3">
              <span
                ...
                >2</span
              >
              <span>Complete sign-in, then return here and save the session.</span>
            </li>
          </ol>
      ...
      <aside class="window-alert-warning flex items-start gap-3 p-4 text-sm leading-5">
        <CircleAlert class="mt-0.5 size-4 shrink-0 text-amber-500" aria-hidden="true" />
        <p class="m-0">
          YouTube does not officially support yt-dlp sessions. Your account can be restricted or
          banned. Use a secondary account and disconnect it when you no longer need authenticated
          playback.
        </p>
      </aside>

      <p v-if="errorMessage" class="window-alert-danger m-0 px-4 py-3 text-sm" role="alert">
        {{ errorMessage }}
      </p>
      <Button
        v-if="errorMessage"
