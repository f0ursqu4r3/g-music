...
describe("MetadataRefreshDrawer", () => {
  it("shows real completion progress, puts active work first, and collapses completed jobs", async () => {
    ...
    expect(wrapper.get("[data-refresh-summary]").text()).toContain(
      "1 refreshed",
    );
    expect(wrapper.get("[data-refresh-summary]").text()).toContain("1 failed");
    expect(
      wrapper
        .findAll("[data-refresh-track-id]")[0]
        .attributes("data-refresh-track-id"),
    ).toBe("active");
    expect(
      wrapper.get('[data-refresh-group="completed"]').attributes("open"),
    ).toBeUndefined();
    ...
    await wrapper
      .get('button[aria-label="Close metadata refresh"]')
      .trigger("click");
  ...
  it("skipped jobs appear in a dedicated Skipped section, not in refreshed or failed counts", () => {
    ...
    expect(wrapper.get("[data-refresh-summary]").text()).not.toContain(
      "failed",
    );
    // skipped count shown
    expect(wrapper.get("[data-refresh-summary]").text()).toContain("2 skipped");
    // individual tracks are listed in the skipped group
    expect(wrapper.get('[data-refresh-track-id="skip1"]').text()).toContain(
  ...
  it("does not show Retry failed button when only skipped jobs are present (no real failures)", () => {
    ...
    expect(
      wrapper.find('button[aria-label="Retry failed metadata"]').exists(),
    ).toBe(false);
  });
  ...
  it("header status text says 'Finished with errors' when only real failures, not when only skipped", () => {
    ...
    expect(wrapperSkipOnly.get('[role="status"]').text()).not.toContain(
      "Finished with errors",
    );
    expect(wrapperSkipOnly.get('[role="status"]').text()).toContain(
      "Finished with skipped tracks",
    );
    expect(
      wrapperSkipOnly.get("progress").attributes("aria-valuetext"),
    ).toContain("1 skipped");
    expect(
      wrapperSkipOnly.get('[data-refresh-group="skipped"] summary').text(),
    ).toContain("Skipped");
  });
