...
describe('QueueWindow', () => {
  it('reserves content-sized duration space before row actions for long tracks', () => {
    ...
    expect(row.classes()).toContain('grid-cols-[2.25rem_minmax(0,1fr)_max-content_2.25rem]')
  ...
  it('keeps upcoming row actions hidden independently of disabled styling while advancing', async () => {
    ...
    for (const state of [
      ...
      expect(current.element.parentElement?.classList.contains('opacity-0')).toBe(false)
      for (const button of wrapper.findAll('[data-current="false"] button')) {
        ...
        expect(shell.classList.contains('group-focus-within:opacity-100')).toBe(true)
    ...
    await wrapper.get('[aria-label="Remove Sugar from queue"]').trigger('click')
  ...
  it('renders a virtual Motion queue and routes transport and playback', async () => {
    ...
    expect(wrapper.find("[data-queue-scroll] [data-slot='scroll-area-viewport']").exists()).toBe(
      true,
    )
    expect(wrapper.get('[data-queue-virtualizer]').attributes('role')).toBe('list')
    ...
    expect(toxicity.find('[aria-label="Drag to reorder"]').exists()).toBe(false)
  ...
  it('shows an empty queue without inactive queue actions', () => {
    ...
    expect(wrapper.find('[aria-label="Queue playback controls"]').exists()).toBe(false)
  })
  ...
  it('shows the current item first and hides completed items', () => {
    ...
    expect(wrapper.get('[data-queue-item][data-queue-index="0"]').text()).toContain('Toxicity')
  ...
  it('shows the active shuffled playback order', async () => {
    ...
    expect(wrapper.get('[data-queue-item][data-queue-index="1"]').text()).toContain('Sugar')
    expect(wrapper.get('[data-queue-item][data-queue-index="2"]').text()).toContain('Toxicity')

    await wrapper.get('button[aria-label="Remove Toxicity from queue"]').trigger('click')
  ...
  it('mounts only visible queue rows for a large queue', () => {
    ...
    expect(wrapper.findAll('[data-queue-item]').length).toBeLessThan(largeQueue.length)
    expect(wrapper.get('[data-queue-virtualizer]').attributes('style')).toContain('height: 5200px')
  })
  ...
  it('commits Motion reorder output through the authoritative queue command', async () => {
    ...
    const reorderGroup = wrapper.getComponent(ReorderGroup) as unknown as MotionReorderHarness
    const reorderItems = wrapper.findAllComponents(ReorderItem) as unknown as MotionReorderHarness[]
  ...
  it('removes only an upcoming item through a visible queue action', async () => {
    ...
    expect(wrapper.find('button[aria-label="Remove Aerials from queue"]').exists()).toBe(false)
    await wrapper.get('button[aria-label="Remove Toxicity from queue"]').trigger('click')
  ...
  it('opens queue context actions without offering current-item removal', async () => {
    ...
    expect(document.body.querySelector('[data-queue-context-menu]')?.textContent).not.toContain(
      'Remove from queue',
    )
    wrapper.unmount()
  })
