...
describe("AutoScrollText", () => {
  it("duplicates and animates text only when it overflows", async () => {
    ...
    Object.defineProperty(
      wrapper.get(".auto-scroll-content").element,
      "scrollWidth",
      {
        configurable: true,
        value: 360,
    ...
    expect(
      wrapper.findAll(".auto-scroll-segment")[1].attributes("aria-hidden"),
    ).toBe("true");
  });
  ...
  it("fades only the edge where moving text is clipped", () => {
    expect(autoScrollTextSource).toContain(
      "animation-name: auto-scroll-edge-fade;",
    );
    expect(autoScrollTextSource).toMatch(
      /0%,\s*14%,\s*86%,\s*100%\s*\{[\s\S]*?black 0/,
    );
    expect(autoScrollTextSource).toMatch(
      /15%,\s*85%\s*\{[\s\S]*?transparent 0/,
    );
    expect(autoScrollTextSource).toContain(
