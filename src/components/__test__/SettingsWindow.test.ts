...
describe('SettingsWindow', () => {
  ...
  it('shows readiness and recovery instructions, refreshes diagnostics, and copies support info', async () => {
    ...
    await wrapper.get('button[aria-label="Refresh diagnostics"]').trigger('click')
    await flushPromises()
    expect(playbackApi.inspectDiagnostics).toHaveBeenCalledTimes(2)
    await wrapper.get('button[aria-label="Copy support info"]').trigger('click')
    await flushPromises()
    expect(writeText).toHaveBeenCalledWith(expect.stringContaining('"appVersion": "1.0"'))
  ...
  it('exports a backup, keeps its path selectable, and shows copy failures', async () => {
    ...
    await flushPromises()
    expect((wrapper.get('input[aria-label="Backup path"]').element as HTMLInputElement).value).toBe(
      '/backups/library.sqlite3',
    )
  ...
  it('opens the isolated YouTube login flow and saves its session', async () => {
    ...
    await wrapper.get('button[aria-label="Sign in to YouTube"]').trigger('click')
    expect(youtubeAuthApi.openLogin).toHaveBeenCalledOnce()

    await wrapper.get('button[aria-label="Use signed-in session"]').trigger('click')
    await flushPromises()

    expect(youtubeAuthApi.saveSession).toHaveBeenCalledOnce()
    expect(wrapper.get('[data-auth-state="connected"]').text()).toContain('Connected')
  })
  ...
  it('disconnects and deletes the dedicated session', async () => {
    ...
    await wrapper.get('button[aria-label="Disconnect YouTube"]').trigger('click')
    await flushPromises()
