...
describe("library hardening", () => {
  it("retains a failed new playlist name and saves it on retry", async () => {
    ...
    failed = false;
    await wrapper
      .get('button[aria-label="Save new playlist"]')
      .trigger("click");
    await flushPromises();
  ...
  it("searches all metadata without losing playlist filters and plays visible order", async () => {
    ...
    for (const [query, ids] of [
      ...
      expect(
        wrapper
          .findAll("[data-track-id]")
          .map((row) => row.attributes("data-track-id")),
      ).toEqual(ids);
    }
    ...
    await wrapper
      .get('button[aria-label="Clear library search"]')
      .trigger("click");
  ...
  it("uses keyboard radio menu selection and returns focus on Escape", async () => {
    ...
    await flushPromises();
    expect(document.activeElement?.getAttribute("data-sort")).toBe(
      "title-desc",
    );
