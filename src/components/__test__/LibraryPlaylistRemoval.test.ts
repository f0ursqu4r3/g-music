...
describe("playlist track removal", () => {
  it.each(["List", "Grid"])(
    "removes only playlist membership from the %s view",
    async (view) => {
      ...
      expect(
        document.body.querySelector('[data-track-context-action="remove"]')
          ?.textContent,
  ...
  it("removes the full selection without deleting an empty playlist", async () => {
    ...
    await wrapper
      .get('[data-track-id="track-c"]')
      .trigger("click", { shiftKey: true });
    await wrapper.get('[data-track-id="track-b"]').trigger("contextmenu");
    expect(removalAction()?.textContent).toContain(
      "Remove 3 tracks from playlist",
    );
    removalAction()!.click();
    await flushPromises();
  ...
  it.each([null, "favorites", "most-played"])(
    ...
    async (playlistId) => {
      ...
      if (playlistId)
        await wrapper
          .get(`[data-playlist-id="${playlistId}"]`)
          .trigger("click");
      await wrapper.get('[data-track-id="track-a"]').trigger("contextmenu");
      expect(
        document.body.querySelector("[data-track-context-menu]"),
      ).not.toBeNull();
