  ...
  vi.mocked(invoke)
    ...
    .mockImplementation(async (command, args) => {
      ...
      if (command === "toggle_shuffle")
        state = { ...state, shuffleEnabled: !state.shuffleEnabled };
      if (command === "cycle_repeat_mode")
        state = { ...state, repeatMode: "all" };
...
describe("real App command routing", () => {
  it("applies lightweight volume events without reloading the library", async () => {
    ...
    expect(
      app
        .get('[role="slider"][aria-label="Volume"]')
  ...
  it("retries the failed native Import window action", async () => {
    ...
    expect(
      vi
        .mocked(invoke)
        .mock.calls.filter(([command]) => command === "show_import_window"),
    ).toHaveLength(2);
  });
  it.each(["playback-updated", "import-progress"])(
    ...
    async (failedEvent) => {
      ...
      expect(app.find("[data-sonner-toast] button[data-action]").exists()).toBe(
        true,
      );
      vi.mocked(listen).mockImplementation(async (name, handler) => {
        events.set(name, handler as (event: { payload: unknown }) => void);
  ...
  it("applies a native queue-clearing event to the real Queue window", async () => {
    ...
    await flushPromises();
    expect(app.find('button[aria-label="Play A session"]').exists()).toBe(
      false,
    );
    expect(
  ...
  it("keeps a metadata draft when the actual IPC save fails and retries that draft", async () => {
    ...
    expect(
      new DOMWrapper(document.body).get('[role="dialog"]').text(),
    ).toContain("Disk full");
    vi.mocked(invoke).mockResolvedValueOnce({
  ...
  it.each(["inspect_library", "inspect_playback"])(
    ...
    async (failedCommand) => {
      ...
      vi.mocked(invoke).mockImplementation(async (command, args) => {
        if (command === failedCommand && failures-- > 0)
          throw new Error("Library is unavailable");
        return normal(command, args);
      });
      const app = await open();
      expect(app.get("[data-sonner-toast]").text()).toContain(
      ...
      expect(app.find('[aria-label="Loading music window"]').exists()).toBe(
        false,
      );
      await app.get("[data-sonner-toast] button[data-action]").trigger("click");
      await flushPromises();
      expect(app.find('button[aria-label="Dismiss error"]').exists()).toBe(
        false,
      );
      expect(
        app
          .find('[data-sonner-toast][data-removed="false"] button[data-action]')
          .exists(),
      ).toBe(true);
      await app.get("[data-sonner-toast] button[data-action]").trigger("click");
      await flushPromises();
  ...
  it("persists Library footer favorite through the actual composable and IPC", async () => {
    ...
    expect(
      app.get('button[aria-label="Favorite track"]').attributes("aria-pressed"),
    ).toBe("true");
  });
  it("routes Artwork shuffle, repeat and favorite using authoritative snapshots", async () => {
    ...
    expect(
      app
        .get('button[aria-label="Disable shuffle"]')
  ...
  it("does not hijack Space on a button or a prevented keyboard event", async () => {
    ...
    vi.mocked(invoke).mockClear();
    await app
      .get('button[aria-label="Import music"]')
      .trigger("keydown", { key: " " });
  ...
  it.each(["library", "mini", "artwork", "queue", "import", "settings"])(
    ...
    async (view) => {
      ...
      await flushPromises();
      expect(
        app.text().split("Audio output failed. Retry playback."),
      ).toHaveLength(2);
      expect(app.find("[data-sonner-toast] button[data-action]").exists()).toBe(
        true,
      );
  ...
  it("receives a theme change from another window and removes the listener on unmount", async () => {
    ...
    window.dispatchEvent(
      new StorageEvent("storage", { key: "gmusic-theme", newValue: "ember" }),
    );
    await flushPromises();
    ...
    wrapper = undefined;
    window.dispatchEvent(
      new StorageEvent("storage", { key: "gmusic-theme", newValue: "plum" }),
    );
    await flushPromises();
    expect(document.documentElement.dataset.theme).toBe("ember");
  });
