    ...
    .mockImplementation(async (command, args) => {
      ...
      switch (command) {
        ...
        case 'upsert_playlist':
          library = {
            ...library,
            playlists: [...library.playlists, (args as { playlist: Playlist }).playlist],
          }
          return library
...
describe('App product workflows through real components and IPC', () => {
  ...
  it('shows a startup string error once and retries the failed inspection', async () => {
    ...
    commandOverrides.set('inspect_import_progress', () => Promise.reject(message))
    const app = await open('library')
    expect(app.text().split(message)).toHaveLength(2)
    expect(app.findAll('[data-sonner-toast][data-removed="false"]')).toHaveLength(1)
  ...
  it('shows a structured playback error only once in the library', async () => {
    ...
    await flushPromises()
    expect(app.text().split(message)).toHaveLength(2)
    expect(app.get('[data-sonner-toast] button[data-action]').text()).toBe('Retry')
  ...
  it('keeps the reconnect notification after a failed save and blocks duplicate saves', async () => {
    ...
    await action().trigger('click')
    expect(
      vi.mocked(invoke).mock.calls.filter(([command]) => command === 'save_youtube_session'),
    ).toHaveLength(1)
    reject({ message: 'Finish signing in before saving the session.' })
    await flushPromises()
    expect(app.text()).toContain('Finish signing in before saving the session.')
    expect(action().text()).toBe('Save session and retry')
  ...
  it('updates one Sonner toast when the same playback error repeats', async () => {
    const app = await open('library')
    for (const message of ['Player unavailable', 'Player unavailable', 'Output disconnected']) {
      events.get('playback-error')?.({ payload: { code: 'player', message } })
      await flushPromises()
    }
  ...
  it.each(['import', 'library'])(
    ...
    async (view) => {
      ...
      expect(app.get('[aria-label="Import progress"]').text()).toContain('Reading source')
      await app.get('button[aria-label="Cancel import"]').trigger('click')
      await flushPromises()
  ...
  it.each([null, 'stale'])(
    ...
    async (result) => {
      ...
      await flushPromises()
      expect(invoke).toHaveBeenCalledWith('inspect_import_progress')
      finish(result === null ? null : { ...progress, runId: 83, message: 'Old run' })
      await flushPromises()
      expect(app.get('[role="log"]').text()).toContain('Reading source')
  ...
  it('searches text, shows provider metadata, and imports only after an explicit action', async () => {
    const app = await open('import')
    await app.get('input[aria-label="Search YouTube"]').setValue('  artist recording  ')
    ...
    expect(app.get('[data-search-result="found"]').text()).toContain('Search artist')
    expect(app.get('[data-search-result="found"]').text()).toContain('Search album')
    expect(
      vi
        .mocked(invoke)
        .mock.calls.some(
          ([command]) => command === 'import_youtube_urls' || command === 'play_track',
        ),
    ).toBe(false)
    await app.get('button[aria-label="Import Discovered recording"]').trigger('click')
    await flushPromises()
    ...
    expect(app.get('[role="log"]').text()).toContain('Reading source')
    expect(
      app.get('button[aria-label="Import Discovered recording"]').attributes('disabled'),
    ).toBeDefined()
  })
  ...
  it('blocks duplicate cancellation while the backend is cancelling', async () => {
    ...
    await flushPromises()
    await app.get('button[aria-label="Cancel import"]').trigger('click')
    expect(app.get('button[aria-label="Cancel import"]').attributes('disabled')).toBeDefined()
    await app.get('button[aria-label="Cancel import"]').trigger('click')
    expect(
      vi.mocked(invoke).mock.calls.filter(([command]) => command === 'cancel_youtube_import'),
    ).toHaveLength(1)
    finish()
    await flushPromises()
  })
  ...
  it('retries failed enrichment through the real composable and updates the visible state', async () => {
    ...
    const app = await open('import')
    await app.get('button[aria-label="Retry failed metadata"]').trigger('click')
    await flushPromises()
    expect(invoke).toHaveBeenCalledWith('retry_metadata_refreshes')
    expect(app.find('button[aria-label="Retry failed metadata"]').exists()).toBe(false)
    expect(app.get('[aria-label="Metadata refresh"]').text()).toContain('0 of 1 refreshed')
  })
  ...
  it('keeps a failed URL import draft and retries the same source through IPC', async () => {
    ...
    await app.get('form[aria-label="Import music from YouTube"]').trigger('submit')
    await flushPromises()
    expect(app.text()).toContain('Import failed. Check your connection.')
    expect((input.element as HTMLTextAreaElement).value).toBe('https://youtu.be/retain-this')
    await app.get('[data-sonner-toast] button[data-action]').trigger('click')
    await flushPromises()
    expect(
      vi.mocked(invoke).mock.calls.filter(([command]) => command === 'import_youtube_urls'),
    ).toEqual([
  ...
  it('persists Settings theme, refreshes diagnostics, and creates a selectable backup path', async () => {
    ...
    await flushPromises()
    expect(
      vi.mocked(invoke).mock.calls.filter(([command]) => command === 'inspect_diagnostics'),
    ).toHaveLength(2)
    await app.get('button[aria-label="Back up library"]').trigger('click')
    await flushPromises()
    expect(invoke).toHaveBeenCalledWith('export_library_backup')
    expect((app.get('input[aria-label="Backup path"]').element as HTMLInputElement).value).toBe(
      '/private/backups/library.sqlite3',
    )
  ...
  it('saves the authoritative queue playback order through a named playlist dialog', async () => {
    ...
    const app = await open('queue')
    await app.get('button[aria-label="Save queue as playlist"]').trigger('click')
    await flushPromises()
  ...
  it('keeps a failed queue playlist draft and retries the same ordered playlist', async () => {
    ...
    expect((dialog().get('input').element as HTMLInputElement).value).toBe('Keep my draft')
    const firstSave = vi
      .mocked(invoke)
      .mock.calls.find(([command]) => command === 'upsert_playlist')
    await dialog().get('form').trigger('submit')
    await flushPromises()
    const saves = vi.mocked(invoke).mock.calls.filter(([command]) => command === 'upsert_playlist')
