...
// The runtime owns this opaque object. An empty enum is uninhabited and
// cannot describe a foreign static. Only its address is used below.
type Class = c_void;
...
impl<A, R, F> ConcreteBlock<A, R, F> {
    ...
    unsafe fn with_invoke(invoke: unsafe extern "C" fn(*mut Self, ...) -> R,
        ...
        ConcreteBlock {
            base: BlockBase {
                isa: ptr::addr_of!(_NSConcreteStackBlock),
